...
if [[ $build_process = binaries || $build_process = rpms ]] ; then
  ...
  if [[ $should_exit_branch = false ]] ; then
    ...
    if [[ $build_process = binaries ]] ; then
      echo "Builiding the binaries on $(nproc) cores for the branch $c4p_condor_branch"
    else
      echo "Builiding the rpms on $(nproc) cores for the branch $c4p_condor_branch"
    fi
  fi
fi
...
if [[ $build_process = binaries ]] ; then
  ...
  binaries_dir="$HOME/C4P-HTCondor/c4p-htcondor-binaries"
  ...
  rpms_dir="$HOME/C4P-HTCondor/c4p-htcondor-rpms"
